...
% D^{attr}, optionally qualified by component:
%   \mAttr        -> D^{attr}
%   \mAttr[heat]  -> D^{attr,heat}
% Written with an optional argument rather than a trailing ^{...} at each use
% site, because \mAttr already carries a superscript and a second one is a
% "Double superscript" error.
\NewDocumentCommand{\mAttr}{o}{%
  \IfNoValueTF{#1}{D^{\text{attr}}}{D^{\text{attr},\,\text{#1}}}%
}
...
% style=nextline: the monospace filenames are wider than any fixed label
% column, so put each on its own line with the description underneath
% (multiline + leftmargin=4.2cm let long labels run into the text).
\begin{description}[style=nextline,leftmargin=1.5em]
  ...
  \item[\texttt{docs/validation/pipeline-math-verification.md}] Row-by-row independent
    re-derivation of the per-pixel math.
  \item[\texttt{docs/ops/pipeline-state-for-cckp.md}] Pipeline architecture,
    script ordering, input file formats.
  \item[\texttt{docs/ops/caspar-workflow.md}] Step-by-step production run
    instructions.
\end{description}
...
$X$ & Scenario (SSP1-RCP2.6, SSP2-RCP4.5, SSP3-RCP7.0, SSP5-RCP8.5) \\
...
\textbf{On the draw index.} When the pipeline runs in draws mode
(\texttt{USE\_DRAWS = TRUE}, $N = 500$), every risk quantity below
($\rho$, $\tilde\rho$, $\PAF$, $\tilde S$, $\mIHME$, $\mAttr$) carries a
draw index $d$. Part~I suppresses $d$ in the pipeline equations because
every operation there is elementwise within a single draw: draw $d$ never
interacts with draw $d' \neq d$. Workflow~B (Section~\ref{sec:workflowb})
carries $d$ explicitly, because there the pairing between IHME's draws and
the pipeline's draws is a methodological choice rather than a
bookkeeping detail. In summary mode (\texttt{USE\_DRAWS = FALSE}) the draw
dimension is absent and every formula reads as written with $d$ dropped.
...
1,034 GBD locations and three time points (1990, 2010, 2020). Study years
without an exact match take the nearest available year's TMREL
(\texttt{02\_load\_tmrel.R}), so the 2022--2050 projection horizon uses
the 2020 TMREL throughout.\footnote{Decadal TMREL projections beyond 2020
are an open methodology choice; the IHME GBD 2021 forecast appendix
Section~2.1.6.7 discusses a 10-year rolling mean approach. Within our
pipeline the cross-scenario ratio cancels first-order TMREL drift, since
both numerator and denominator use the same TMREL.}

Under the \texttt{COLOMBIA\_VERIFICATION} flag the pipeline instead takes
an equal-weight mean of all three source years and applies that single
zone-specific TMREL across the study period, reproducing Samuel Cuervo's
choice. That branch exists only for the Colombia validation of
Section~\ref{sec:validation}; production runs use the nearest-year path
described above.
...
(SSP3-RCP7.0, SSP5-RCP8.5, SSP1-RCP2.6).\footnote{The low-emissions
target is SSP1-RCP\textbf{2.6} (\texttt{ssp126}), not SSP1-RCP1.9: the
CCKP daily per-model product carries no \texttt{ssp119} for any of its 29
models. There, \texttt{ssp119} exists only as an annual multi-model-ensemble
climatology, which this daily per-model pipeline cannot consume. The four
production scenarios, in run-priority order, are \texttt{ssp245}
(reference), \texttt{ssp370}, \texttt{ssp585}, \texttt{ssp126}; the
reference runs first because it is the Workflow~B denominator for every
target.} IHME does not publish per-SSP forecasts. Our task: take the published $\SSPtwo$-anchored mortality and
...
\begin{align}
\mAttr[cold]_{c,l,a,s,t} &= m_{c,l,a,s,t} \cdot \PAF^{\text{cold}}_{c,l,t} \label{eq:attrib-cold} \\
\mAttr[heat]_{c,l,a,s,t} &= m_{c,l,a,s,t} \cdot \PAF^{\text{heat}}_{c,l,t} \label{eq:attrib-heat}
\end{align}
...
the scale factor, formed within a draw:
\begin{equation}
r_{c,X,l,t,d} = \frac{\tilde S_{c,X,l,t,d}}{\tilde S_{c,\SSPtwo,l,t,d}}
\label{eq:ratio}
\end{equation}
Then for the 12 temp-scaled causes:
\begin{equation}
\mAttr[heat]_{c,X,l,a,s,t,d} = \mIHME_{c,l,a,s,t,d} \cdot r_{c,X,l,t,d} \cdot \PAF^{\text{heat}}_{c,X,l,t,d}
\label{eq:wb-12}
\end{equation}
...
absolute calibration?'' (This subsection suppresses the draw index $d$;
the argument holds within each draw separately.)
...
\begin{equation}
\mAttr[heat]_{c \in \text{non-temp}, X, l, a, s, t, d}
= \mIHME_{c,l,a,s,t,d} \cdot \PAF^{\text{heat}}_{c,X,l,t,d}
\label{eq:wb-5}
\end{equation}
...
\subsection{Draws and the pairing convention}
\label{sec:draws}

Both inputs to Eq.~\eqref{eq:wb-12} are uncertain, and both are published
as draws: IHME's forecast mortality carries 500 draws per
$(c, l, a, s, t)$ cell, and our pipeline propagates ERF and TMREL
uncertainty into $\tilde S$ and the PAFs.

We combine them by \textbf{pairing on the draw index}: IHME draw $d$ is
multiplied by pipeline draw $d$, for $d = 0, \ldots, 499$. We do
\emph{not} take the draw $\times$ draw cartesian product. This is the GBD
convention, and it is why $d$ appears once (not twice) in
Eqs.~\eqref{eq:ratio}--\eqref{eq:wb-5}. The output therefore has 500
draws rather than $500^2 = 250{,}000$, and the reported interval reflects
IHME's forecast uncertainty and our pipeline's exposure-response
uncertainty combined draw-wise.

Two consequences worth stating plainly:
\begin{itemize}[itemsep=2pt]
  \item The numerator and denominator of Eq.~\eqref{eq:ratio} use the
    \emph{same} pipeline draw $d$. This is what lets the calibration
    factor $k$ cancel within a draw, as argued above; ratio-ing draw $d$
    against draw $d'$ would reintroduce the very drift the ratio is meant
    to remove.
  \item Pairing by index is a convention, not a claim that IHME's draw
    $d$ and our draw $d$ correspond to the same underlying state of the
    world. They are separate posterior samples from separate models. The
    pairing keeps the arithmetic within-draw and the draw count
    manageable; it does not assert a shared parameterization.
\end{itemize}

In summary mode (\texttt{USE\_DRAWS = FALSE}) the burdens have no draw
column, $\tilde S$ and the PAFs collapse to point estimates, and
Workflow~B returns a point estimate with no interval. The implementation
branches on the presence of a \texttt{draw} column in
\texttt{util\_workflow\_b\_ratio.R}: when present, it joins on
$(\text{year}, \text{subloc}, \text{cause}, \text{draw})$; when absent, on
$(\text{year}, \text{subloc}, \text{cause})$.
...
\label{sec:trace-one-pixel}
% =====================================================================

We trace pixel 527 in Colombia, a Bogotá D.C. metro pixel (subloc 11),
in 2010. Following the math of Section~\ref{sec:pipeline-math}:
...
\texttt{docs/validation/pipeline-math-verification.md}).

% =====================================================================
\section{Within-pipeline trace: contrasting pixels}
\label{sec:trace-contrasting}
% =====================================================================
...
\begin{align*}
\mAttr[non-opt]_{\text{cvd\_ihd}, \SSPtwo, 2030}
...
values). Then:
\begin{align*}
\mAttr[heat]_{\text{cvd\_cmp, SSP5, 2030}}
&= 5{,}000 \cdot 0.004 = 20 \text{ deaths} \\
\mAttr[cold]_{\text{cvd\_cmp, SSP5, 2030}}
...
pipeline's own saved values (Sections~\ref{sec:trace-one-pixel} and
\ref{sec:country-agg} above; full details in
\texttt{docs/validation/pipeline-math-verification.md}) confirms every formula in
Section~\ref{sec:pipeline-math} is correctly implemented: 9 calculation
stages match to 8+ decimal places, and the within-subloc aggregation
reproduces the pipeline's saved per-subloc PAF (0.06956161 vs 0.06956161
for Bogotá D.C. cvd\_ihd cold, 2010).

\subsection*{The Workflow~B draws path}

The draw plumbing of Section~\ref{sec:draws} is checked by a self-ratio:
setting the target scenario equal to the reference ($X = \SSPtwo$) makes
$r_{c,X,l,t,d} = 1$ identically, so Eq.~\eqref{eq:wb-12} must collapse to
$\mIHME \cdot \PAF$, which is the pipeline's own burden. It does. For
Colombia (location 125) in 2022, using IHME national forecast mortality
and \texttt{access-cm2-r1i1p1f1} \texttt{ssp245} temperature,
$\mAttr[non-opt]$ has mean 1{,}721 deaths with a 95\% draw
interval of $[978,\ 2{,}417]$ across 500 draws, matching the burden, and
\texttt{scale\_factor} is exactly 1 on every row.

This exercises the draw pairing and the join, but \emph{not} the ratio
itself: a self-ratio holds $r = 1$ by construction. The case $r \neq 1$
has not yet been run on real data, because no target-scenario production
run exists at the time of writing. Section~\ref{sec:wb-example} remains
...
generated the values in
Sections~\ref{sec:trace-one-pixel}--\ref{sec:country-agg} is
...
\texttt{docs/ops/caspar-workflow.md} in the project repository.
