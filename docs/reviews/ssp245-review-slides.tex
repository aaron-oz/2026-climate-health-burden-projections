...
\begin{frame}{Bottom line}
  \begin{itemize}
    ...
    \item An apparent large shortfall in hot countries turned out to be
      \ozcyantext{GBD's own upward revision between its 2019 and 2023 releases}, not a pipeline
      defect.
    \item Main quantified limitations: risk curves end at
      32.1\,\textcelsius{}; a tropical cluster running low vs.\ GBD 2019
      for reasons not yet identified (temperature bias and damped daily
      variability both tested and ruled out); a YLL definition that differs
      from GBD's.
...
\begin{frame}{All quality gates pass}
  \begin{itemize}
    ...
    \item Coverage complete: every country has all 27 models and all 29
      years.
    \item All 159{,}732 combinations carry all 17 causes and YLLs.
  \end{itemize}
\end{frame}

\ozsection{Validation against published estimates}

\begin{frame}{Anchor 1: the nine countries in Burkart 2021's Table 1}
  \centering\small
  \begin{tabular}{lrrr}
    \toprule
...
\begin{frame}{Anchor 2: all 204 countries vs.\ the GBD 2019 estimates}
  ...
  \end{exampleblock}
  \begin{itemize}
    \item Source: the GBD 2019 temperature-attributable burden dataset,
      published alongside Burkart 2021 (IHME GHDx, DOI 10.6069/7QWW-PF74)
      and verified against Burkart's Table 1 (PAFs agree to \(\le 0.015\)
      percentage points).
    \item Intervals that fail to overlap (their UI vs.\ our draw interval):
      one country, Cape Verde; the other 44 sit within joint uncertainty.
...
\begin{frame}{The apparent hot-country shortfall vs.\ GBD 2023}
  \begin{itemize}
    \item Against GBD 2023's published 2022 estimates, our
...
\begin{frame}{Resolution: it is GBD's between-round revision}
  ...
  \begin{tabular}{lrrr}
    \toprule
    country & vs.\ GBD 2023 & vs.\ GBD 2019 & GBD's own revision \\
    \midrule
  ...
  revised these countries upward by 1.3--2.3\(\times\) between releases.
  Whether to track the Burkart 2021 methodology or GBD 2023 is a scoping
...
\begin{frame}{Trends make sense once composition is removed}
  \begin{itemize}
    \item Raw cold-attributable deaths \emph{rise} in most countries
      (1.33M in 2022 to 1.99M in 2050), which looks wrong: warming means
      fewer cold days, so cold deaths should fall.
    \item Two composition effects explain it: population aging, and the
      ...
      latitudes.
    \item After age-sex standardization (attributable share of deaths,
      holding each country's 2022 age mix fixed): heat share rises in 197
      of 204 countries; cold share falls in 140 of 204.
  \end{itemize}
...
\begin{frame}{Model spread and the YLL definition}
  \begin{itemize}
    ...
    \item YLLs per attributable death: ours 13.9 years vs.\ GBD's 21.2.
      Definitional: GBD uses its reference life table (21.29 years remaining
      at age 70); we use national UN life expectancies. Applying GBD's table
      to our attributable-death ages gives 17.7--21.5 years, spanning their
      21.2. State the definition wherever YLLs are compared.
  \end{itemize}
\end{frame}
...
\begin{frame}{Risk curves end at a daily mean of 32.1\,\textcelsius}
  \begin{itemize}
    \item The Burkart ERF curves exist only over each zone's observed range;
...
\begin{frame}{The tropical low cluster: leading hypothesis tested and rejected}
  \begin{itemize}
    \item Daily input is NASA NEX-GDDP-CMIP6 (CMIP6 quantile-mapped to
      GMFD): not raw CMIP6, not the ERA5 behind the GBD 2019 estimates.
    \item Suspect: damped day-to-day spread. Tested against ERA5 2022 in 15
      countries, covering 8 of the 23 low-cluster members directly.
    \item \alert{Rejected as the general explanation}: spread ratios
      0.88--1.18 for 7 of 8 (Uganda 0.74), spread-vs-burden correlation
      0.15. Haiti: spread ratio 0.92, burden ratio 0.35. Cause remains
      unidentified.
    \item Surfaced sensitivity: the two observational references disagree
      by 1.9\,\textcelsius{} for the Philippines; switching moves its
      burden \(\sim\)2\(\times\). No published uncertainty interval
      includes that choice.
  \end{itemize}
\end{frame}
...
\begin{frame}{Six recommendations}
  \begin{enumerate}
    ...
    \item State the alignment target in all outputs: these are estimates
      under the Burkart 2021 methodology. If GBD 2023 alignment is
      wanted later, obtain the GBD 2023 curves from IHME; do not tune
      ours.
...
\begin{frame}{Provenance}
  \begin{itemize}
    \item Production summary tables: August 14, 2026, produced by Caspar.
    \item Review analyses: August 14--18, 2026; notes in
      \texttt{docs/reviews/ssp245-review-findings.org}, scripts and outputs in
      \texttt{output/review-ssp245/} in the project repository.
    \item External comparators: Burkart et al.\ (2021) Table 1; IHME GHDx
      dataset DOI 10.6069/7QWW-PF74 (the GBD 2019 estimates); GBD
      Results Tool exports (GBD 2023), July 31, 2026.
